...
def infer_multi_output(data, location):
    ...
    formatted_values = [int(round(value)) for value in predicted_values]
    ...
    try:
        ...
        pred_query = f'''
            from(bucket: "{bucket}")
                ...
                |> filter(fn: (r) => r._measurement == "pred_vehicles_count")
...
def write_predicted_data(prediction, latest_time, location):
    ...
    try:
        ...
        pred_point = (
            Point("pred_vehicles_count")  # Measurement name
            .field("vehicles_coming_in", prediction[0])  
            .field("vehicles_going_out", prediction[1])  
    ...
    while True:  
        try:
            ...
            if len(latest_data) == 7 and latest_time != datetime.utcnow().date() + timedelta(days=7):
                ...
                prediction = infer_multi_output(latest_data, location)
