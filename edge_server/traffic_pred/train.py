...
bucket = "traffic_camera_data"  # Bucket name
...
def fetch_latest_data(location):
    ...
    from(bucket: "{bucket}")
        ...
        |> filter(fn: (r) => r._measurement == "vehicles_count")
    ...
    try:
        ...
        for table in result:
            for record in table.records:
                points.append({
                    "vehicles_coming_in": record["vehicles_coming_in"],
                    "vehicles_going_out": record["vehicles_going_out"],
                    "time": record["_time"]
                })
        ...
        for point in points:
            processed_points.append({
                "vehicles_coming_in": point["vehicles_coming_in"],
                "vehicles_going_out": point["vehicles_going_out"]
            })
...
def train_model(data, location):
    ...
    scaler = MinMaxScaler()
    scaled_data = scaler.fit_transform(data[['vehicles_coming_in', 'vehicles_going_out']])

    os.makedirs(f"traffic_pred_models/{location}", exist_ok=True)

    # Save the scaler
    joblib.dump(scaler, f"traffic_pred_models/{location}/scaler.pkl")

    # Replace the original data with the scaled data
    data[['vehicles_coming_in', 'vehicles_going_out']] = scaled_data
    ...
    model_checkpoint = ModelCheckpoint(f'traffic_pred_models/{location}/model.h5', monitor='val_loss', save_best_only=True)
    ...
    plt.plot(true_values[-100:, 0], label='Actual vehicles_coming_in')
    plt.plot(predicted_values[-100:, 0], label='Predicted vehicles_coming_in')
    plt.plot(true_values[-100:, 1], label='Actual vehicles_going_out')
    plt.plot(predicted_values[-100:, 1], label='Predicted vehicles_going_out')
    
    plt.legend()
    plt.title("Predicted vs Actual for vehicles_coming_in, vehicles_going_out")
    plt.xlabel('Time')
    plt.ylabel('Value')
    plt.show()
