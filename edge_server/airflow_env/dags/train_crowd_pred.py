...
def train_model(data, location):
    ...
    os.makedirs(f"/opt/airflow/dags/crowd_pred_models/{location}", exist_ok=True)

    # Save the scaler
    joblib.dump(scaler, f"/opt/airflow/dags/crowd_pred_models/{location}/scaler.pkl")
    ...
    model.add(Dropout(0.2))
    model.add(Dense(units=1))  # Output layer output for predicted crowd count
    ...
    model_checkpoint = ModelCheckpoint(f'/opt/airflow/dags/crowd_pred_models/{location}/model.h5', monitor='val_loss', save_best_only=True)
    ...
    best_model = tf.keras.models.load_model(f'/opt/airflow/dags/crowd_pred_models/{location}/model.h5')
...
with DAG(
    ...
    dag_id='train_crowd_pred',
    description='Train Crowd Prediction Model',
