...
def infer_multi_output(data, location):
    ...
    scaler = joblib.load(f"../airflow_env/dags/crowd_pred_models/{location}/scaler.pkl")
    scaled_data = scaler.transform(data)
    reshaped_data = np.expand_dims(scaled_data, axis=0)  # Add a batch dimension

    # Load the trained model
    best_model = tf.keras.models.load_model(f"../airflow_env/dags/crowd_pred_models/{location}/model.h5")
...
def write_predicted_data(prediction, latest_time, location):
    ...
    try:
        ...
        pred_point = (
            Point("pred_people_count")  # Measurement name
            .field("count", prediction[0])  
